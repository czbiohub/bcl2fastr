...
use log::{debug, info};
use ndarray::{Array2, Array3, ArrayView1, ArrayView3, Axis, ShapeBuilder};
use rayon::prelude::*;

use crate::extract_reads::extract_cbcl;
use crate::novaseq_run::NovaSeqRun;
use crate::sample_data::{SampleData, Samples};
...
/// Demux a single lane (or all lanes, if they are being combined)
fn demux_lane(
    novaseq_run: &NovaSeqRun,
    lane_n: usize,
    samples: &Samples,
    ...
    compression: u32,
    max_n_pf: usize,
    buffer_array: &mut Array3<u8>,
    index_array: &mut Array3<u8>,
    idx_slices: &[[usize; 2]],
    locs_array: &mut Array2<[u32; 2]>,
    id_array: &mut Array2<Option<(usize, bool)>>,
) -> Result<(), &'static str> {
    ...
    for lane in lane_iter {
        for surface in novaseq_run.run_info.flowcell_layout.surface_range.clone() {
                ...
                .enumerate()
            {
                ...
                f_chunk
                    .par_iter()
                    .zip(locs_array.axis_iter_mut(Axis(0)))
                    .for_each(|(filter, mut locs_vec)| {
                        let mut i = 0;
                            ...
                            novaseq_run.locs.chunks(2).zip(filter.iter().cloned())
                        {
                            match filt {
                                0b01 => {
                                    locs_vec[i] = loc_chunk[1];
                                    i += 1;
                                }
                                0b10 => {
                                    locs_vec[i] = loc_chunk[0];
                                    i += 1;
                                }
                                0b11 => {
                                    locs_vec[i] = loc_chunk[0];
                                    locs_vec[i + 1] = loc_chunk[1];
                                    i += 2;
                                }
                                _ => (),
                            };
                ...
                index_array
                    ...
                    .into_par_iter()
                    .zip(n_pf_chunk)
                    .zip(id_array.axis_iter_mut(Axis(0)))
                    .for_each(|((ix_chunk, &n_pf), mut id_chunk)| {
                        let ix_array = ix_chunk.slice(ndarray::s![.., ..n_pf, 0]);

                        ix_array
                            .axis_iter(Axis(1))
                            .into_par_iter()
                            .zip(id_chunk.as_slice_mut().unwrap())
                            .for_each(|(ix_row, id)| {
                    ...
                    read_files
                        ...
                        .map(|(sample_i, sample_filepath)| {
                            let read_counts: Vec<_> = buffer_array
                                ...
                                .zip(locs_array.axis_iter(Axis(0)))
                                .zip(tid_chunk)
                                .zip(n_pf_chunk)
                                .zip(id_array.axis_iter(Axis(0)))
                                .map(
                                    |(
...
pub fn demux_fastqs(
    ...
    let mut locs_array: Array2<[u32; 2]> = Array2::from_elem((n_tiles, max_n_pf), [0u32; 2]);
    // // preallocate vectors for loc tuples
    // let mut locs_vecs = vec![Vec::with_capacity(max_n_pf); n_tiles];

    let mut id_array: Array2<Option<(usize, bool)>> = Array2::from_elem((n_tiles, max_n_pf), None);
    // // this array will hold sample ids and exact-match bools
    // let mut sample_ids = vec![Vec::with_capacity(max_n_pf); n_tiles];

    for (lane, samples) in sample_data {
        demux_lane(
            &novaseq_run,
            lane.clone(),
            &samples,
...
mod tests {
    ...
    fn demux_fastqs() {
        ...
        super::demux_fastqs(&novaseq_run, &sampledata, &output_path, 2, 1).unwrap();
    }
}
